...
Библиотека блоков, предоставляющая готовые компоненты управления формами и ряд вспомогательных блоков.

Содержит абстрагированные от дизайна базовые реализации, не предоставляющие контентные HTML-модели. Возможно опциональное использование тем.
...
История изменений доступна на [отдельной странице](/CHANGELOG.ru.md).

Миграция на последующие версии описана на [отдельной странице](/MIGRATION.ru.md).
...
$ npm install
```
Для последующего запуска локально установленных `enb` потребуется `export PATH=./node_modules/.bin:$PATH` или любой альтернативный способ.
...
1. Определить версию проекта для внесения изменений.
...
1. Внести изменения. Для версии библиотеки bem-components `v2`, можно локально проверить результаты изменений на наличие ошибок, запустив `npm test`.
...
* [OpenSauce](https://saucelabs.com/opensauce) аккаунт в SauceLabs;
* утилита [Sauce Connect](https://saucelabs.com/connect).

<a name="run-test"></a>
Для запуска тестов необходимо:

1. Настроить окружение (переменные среды `SAUCE_USERNAME` и `SAUCE_ACCESS_KEY`).
1. Запустить утилиту `sc` (SauceConnect) и дождаться установки туннеля.
1. Запустить тесты (команда `npm run gemini`).
1. Собрать новые версии скриншотов при необходимости (команда `npm run gemini-gather`).
